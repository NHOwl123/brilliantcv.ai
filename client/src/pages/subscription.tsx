    ...
    <Card className="max-w-md mx-auto">
      ...
      <CardContent>
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <h4 className="font-semibold text-blue-900 mb-2">Test Card Information</h4>
...
export default function Subscription() {
  ...
  // Handle tier selection from URL parameter or localStorage
  useEffect(() => {
    if (!user) return; // Wait for user to be loaded
    
    const urlParams = new URLSearchParams(window.location.search);
    const tierFromUrl = urlParams.get('tier');
    const tierFromStorage = localStorage.getItem('selectedTier');
    
    if (tierFromUrl && ['free', 'standard', 'premium'].includes(tierFromUrl)) {
      if (tierFromUrl !== 'free') {
        setTimeout(() => handleUpgrade(tierFromUrl), 500); // Delay to ensure user is loaded
      }
      // Clean up URL
      window.history.replaceState(null, "", window.location.pathname);
    } else if (tierFromStorage && ['standard', 'premium'].includes(tierFromStorage)) {
      setTimeout(() => handleUpgrade(tierFromStorage), 500); // Delay to ensure user is loaded
